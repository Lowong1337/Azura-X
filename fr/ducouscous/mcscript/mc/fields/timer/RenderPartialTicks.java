...
public class RenderPartialTicks extends Field<Number> {
    @Override
    protected Number get() {
        return new Number(Minecraft.getMinecraft().timer.renderPartialTicks);
    }

    @Override
    protected void set(Number number) {
        Minecraft.getMinecraft().timer.renderPartialTicks = (float) number.getValue();
    }
}
